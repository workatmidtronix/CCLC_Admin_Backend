...
const createStorage = (uploadType, email) => {
  ...
  destination: function (req, file, cb) {
      ...
      switch (uploadType) {
        ...
        case 'student':
          ...
          break;
        case 'voucher':
          uploadDir = path.join(__dirname, '../../uploads/vouchers', safeEmail);
          break;
        default:
  ...
  filename: function (req, file, cb) {
      ...
      if (uploadType === 'student') {
        ...
        cb(null, fileName);
      } else if (uploadType === 'voucher') {
        // For vouchers, use original filename with timestamp
        const timestamp = Date.now();
        const fileExtension = path.extname(file.originalname);
        const fileName = `voucher_${timestamp}${fileExtension}`;
        cb(null, fileName);
      } else {
...
// Configure multer for voucher file uploads
const createVoucherUpload = () => {
  const storage = createStorage('voucher');
  
  return multer({
    storage: storage,
    fileFilter: fileFilter,
    limits: {
      fileSize: 10 * 1024 * 1024 // 10MB limit for vouchers
    }
  }).fields([
    { name: 'voucherFile', maxCount: 1 }
  ]);
};

// Helper function to get relative path for database storage
const getRelativePath = (uploadType, email, filename) => {
  const safeEmail = email ? email.replace(/[^a-zA-Z0-9]/g, '_') : 'vouchers';
  ...
  switch (uploadType) {
    ...
    case 'voucher':
      return `/uploads/vouchers/${safeEmail}/${filename}`;
    default:
...
const getAbsolutePath = (uploadType, email, filename) => {
  ...
  switch (uploadType) {
    ...
    case 'voucher':
      return path.join(__dirname, '../../uploads/vouchers', safeEmail, filename);
    default:
...
// Helper function to handle voucher file uploads
const handleVoucherFileUpload = () => {
  return createVoucherUpload();
};
