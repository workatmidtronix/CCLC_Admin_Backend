...
router.post('/categories', async (req, res) => {
  try {
    ...
    db.query(query, [course_id, instructor_id || null, category_name, weight], (err, result) => {
      ...
      // Log activity
      try {
        logActivity('Grade Category', 'CREATE', `Created grade category: ${category_name}`, req.user?.id);
      } catch (loggerError) {
...
router.put('/categories/:id', async (req, res) => {
  try {
    ...
    db.query(query, [course_id, instructor_id || null, category_name, weight, id], (err, result) => {
      ...
      // Log activity
      try {
        logActivity('Grade Category', 'UPDATE', `Updated grade category ID: ${id}`, req.user?.id);
      } catch (loggerError) {
        console.log('Activity logger not available:', loggerError.message);
      }
...
router.delete('/categories/:id', async (req, res) => {
  try {
    ...
    // Check if category has associated grade columns
    ...
    db.query(checkQuery, [id], (checkErr, checkResult) => {
      ...
      db.query(deleteQuery, [id], (deleteErr, deleteResult) => {
        ...
        // Log activity
        try {
          logActivity('Grade Category', 'DELETE', `Deleted grade category ID: ${id}`, req.user?.id);
        } catch (loggerError) {
          console.log('Activity logger not available:', loggerError.message);
        }
...
router.post('/columns', async (req, res) => {
  try {
    ...
    db.query(query, [course_id, category_id, column_name, max_points, include_in_calculation || true], (err, result) => {
      ...
      // Log activity
      try {
        logActivity('Grade Column', 'CREATE', `Created grade column: ${column_name}`, req.user?.id);
      } catch (loggerError) {
        console.log('Activity logger not available:', loggerError.message);
      }
...
router.put('/columns/:id', async (req, res) => {
  try {
    ...
    db.query(query, [course_id, category_id, column_name, max_points, include_in_calculation || true, id], (err, result) => {
      ...
      // Log activity
      try {
        logActivity('Grade Column', 'UPDATE', `Updated grade column ID: ${id}`, req.user?.id);
      } catch (loggerError) {
        console.log('Activity logger not available:', loggerError.message);
      }
...
router.delete('/columns/:id', async (req, res) => {
  try {
    ...
    db.query(deleteQuery, [id], (deleteErr, deleteResult) => {
      ...
      // Log activity
      try {
        logActivity('Grade Column', 'DELETE', `Deleted grade column ID: ${id}`, req.user?.id);
      } catch (loggerError) {
        console.log('Activity logger not available:', loggerError.message);
      }
